...
while ( my @row = $rhs_query_sth->fetchrow_array ) {
    my ( $chr, $runlist, $tag ) = @row;
    my $set = AlignDB::IntSpan->new($runlist);
    push @all_rhs, { chr => $chr, set => $set, tag => $tag };
    if ( !exists $chr_rhs_set{$chr} ) {
        $chr_rhs_set{$chr} = AlignDB::IntSpan->new;
    }
    $chr_rhs_set{$chr}->merge($set);
}
