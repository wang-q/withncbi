...
getLayout <-
    function(infoFile, infoCols, heatmapData, barData, doBlocks, treeWidth = 10,
        ...
        m <-
            cbind(c(0, 0, 0), c(0, 1, 0)) # first two columns, edge + tree
        x = 1
...
plotTree <-
    function(tree, ladderise = NULL, heatmapData = NULL, barData = NULL,
        ...
        {
            print(
                "Ladderise option should be exactly 'ascending' or 'descending'.  Any other command will raise this error. Leave option empty to order branches as per input tree."
            )
        }
        ...
        tip.label.order <-
            tl$tip.label[tip.order] # for ordering data. note that for tiplabel(), the order is the same as in t$tip (= tl$tip)
            ...
            if (!is.null(cluster)) {
                if (!(cluster == FALSE)) {
                    if (cluster == "square" & ncol(y.ordered) == nrow(y.ordered)) {
                        ...
                        y.ordered <-
                            y.ordered[, rev(as.numeric(reordered))]
                    }
                    else {
                        # cluster columns
                        if (cluster == TRUE) {
                            cluster = "ward"
                        } # set default clustering algorithm
                        h <- hclust(dist(t(na.omit(
                            y.ordered
                        ))), cluster)
        ...
        if (!is.null(colourNodesBy) & !is.null(infoFile)) {
            if (colourNodesBy %in% colnames(info.ordered)) {
                ...
                loc1 <-
                    info.ordered[, which(colnames(info.ordered) == colourNodesBy)]
        ...
        l <-
            getLayout(
                infoFile, infoCols, heatmapData, barData, doBlocks, treeWidth = treeWidth, infoWidth =
                    infoWidth, dataWidth = dataWidth, edgeWidth = edgeWidth, labelHeight = labelHeight, mainHeight =
                    mainHeight, barDataWidth = barDataWidth, blockPlotWidth = blockPlotWidth
            )
        ...
        tlp <-
            plot.phylo(
                tl, no.margin = T, show.tip.label = tip.labels, label.offset = offset, edge.width = lwd, edge.color = edge.color, xaxs = "i", yaxs = "i", y.lim = c(0.5, length(tl$tip) + 0.5), cex = tipLabelSize
            )
        ...
        if (!is.null(infoFile)) {
            ...
            if (printCols) {
                ...
                plot(
                    NA, axes = F, pch = "", xlim = c(0, length(infoColNumbers) + 1.5), ylim = c(0.5, length(tl$tip) + 0.5), xaxs = "i", yaxs = "i"
                )
                ...
                for (i in 1:length(infoColNumbers)) {
                    j <- infoColNumbers[i]
                    text(
                        x = rep(i + 1, nrow(info.ordered) + 1), y = c((nrow(
                            info.ordered
                        )):1), info.ordered[, j], cex = infoCex
                    )
                }
            }
        }
        ...
        if (!is.null(heatmapData)) {
            if (is.null(heatmapBreaks)) {
                heatmapBreaks = seq(
                    min(y.ordered, na.rm = T), max(y.ordered, na.rm = T), length.out = length(heatmap.colours) + 1
                )
            }
            ...
            if (!is.null(heatmap.blocks)) {
                for (coords in heatmap.blocks) {
                    rect(
                        xleft = coords[1], 0, coords[2], ncol(y.ordered), col = vlines.heatmap.col, border =
                            NA
                    )
                }
            }
            ...
            par(mar = rep(0, 4))
            plot(
                NA, axes = F, xaxs = "i", yaxs = "i", ylim = c(0, 2), xlim = c(0.5, ncol(y.ordered) +
                                                                                   0.5)
            )
            text(
                1:ncol(y.ordered) - 0.5, rep(0, ncol(x)), colnames(y.ordered), srt = 90, cex =
                    colLabelCex, pos = 4
            )
            ...
            image(
                as.matrix(seq(
                    min(y.ordered, na.rm = T), max(y.ordered, na.rm = T), length.out = length(heatmap.colours) +
                        1
                )), col = heatmap.colours, yaxt = "n", breaks = heatmapBreaks, axes = F
            )
            axis(
                1, at = heatmapBreaks[-length(heatmapBreaks)] / max(y.ordered, na.rm = T), labels =
                    round(heatmapBreaks[-length(heatmapBreaks)], heatmapDecimalPlaces)
            )
        }
        ...
        if (!is.null(barData)) {
            par(mar = rep(0, 4))
            barplot(
                barData[tip.label.order], horiz = T, axes = F, xaxs = "i", yaxs = "i", xlab =
                    "", ylab = "", ylim = c(0.25, length(barData) + 0.25), xlim = c((-1) * max(barData, na.rm =
                                                                                                   T) / 20, max(barData, na.rm = T)), col = barDataCol, border = 0, width = 0.5, space =
                    1, names.arg = NA
            )
            ...
            par(mar = c(2, 0, 0, 0))
            plot(
                NA, yaxt = "n", xaxs = "i", yaxs = "i", xlab = "", ylab = "", ylim = c(0, 2), xlim =
                    c((-1) * max(barData, na.rm = T) / 20, max(barData, na.rm = T)), frame.plot =
                    F
            )
        }
        ...
        if (doBlocks) {
            par(mar = rep(0, 4))
            plot(
                NA, axes = F, pch = "", xlim = c(genome_offset, genome_offset + genome_size +
                                                     1.5), ylim = c(0.5, length(tl$tip) + 0.5), xaxs = "i", yaxs = "i"
            ) # blank plotting area
            
            # plot snps
            if (!is.null(snpFile)) {
                snps <-
                    read.csv(snpFile, header = F, row.names = 1) # in case colnames start with numbers or contain dashes, which R does not like as column headers
                snps_strainCols <-
                    snps[1,] # column names = strain names
                snps <- snps[-1,] # drop strain names
                
                for (strain in tip.label.order) {
                    # print SNPs compared to ancestral alleles in column 1
                    s <-
                        rownames(snps)[(as.character(snps[, 1]) != as.character(snps[, which(snps_strainCols ==
                                                                                                 strain)])) &
                    ...
                    if (length(s) > 0) {
                        for (x in s) {
                            points(
                                x, y, pch = "|", col = snp_colour, cex = 0.25
                            )
                        }
                    }
                }
            ...
            if (!is.null(blockFile)) {
                ...
                for (i in 1:nrow(blocks)) {
                    if (as.character(blocks[i, 1]) %in% tip.label.order) {
                        ...
                        x2 <- blocks[i, 3]
                        lines(
                            c(x1, x2), c(y, y), lwd = blwd, lend = 2, col = block_colour
                        )
                    }
                }
            }
        ...
        return(list(
            info = info.ordered, anc = ancestral, mat = mat, strain_order = tip.label.order
        ))
    }
