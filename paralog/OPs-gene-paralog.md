...
runlist stat -s ../data/chr.sizes paralog.yml -o ../stat/paralog.yml.csv
runlist stat -s ../data/chr.sizes paralog_adjacent.yml -o ../stat/paralog_adjacent.yml.csv
...
cat ../yml/repeat.family.txt \
    | parallel -j 1 --keep-order "
        echo \"==> {}\";
        runlist stat2 ../yml/paralog.yml ../yml/{}.yml -s ../data/chr.sizes \
            --op intersect --all \
            -o ../stat/${GENOME_NAME}.paralog.{}.csv
    "
echo "key,chr_length,paralog_size,key_length,key_size,c1,c2,ratio" \
    > ../stat/${GENOME_NAME}.paralog.all-repeat.csv
cat ../yml/repeat.family.txt \
    | parallel -j 1 --keep-order "
        cat ../stat/${GENOME_NAME}.paralog.{}.csv \
            | perl -nl -e '/^chr_length/ and next; print qq({},\$_)'
    " >> ../stat/${GENOME_NAME}.paralog.all-repeat.csv
cat ../yml/repeat.family.txt \
    | parallel -j 1 --keep-order "rm ../stat/${GENOME_NAME}.paralog.{}.csv"

echo "====> paralog_adjacent stats"
cd ~/data/alignment/gene-paralog/${GENOME_NAME}/stat
cat ../yml/repeat.family.txt \
    | parallel -j 1 --keep-order "
        echo \"==> {}\";
        runlist stat2 ../yml/paralog_adjacent.yml ../yml/{}.yml -s ../data/chr.sizes \
            --op intersect --all \
            -o ../stat/${GENOME_NAME}.paralog_adjacent.{}.csv
    "
echo "key,chr_length,paralog_adjacent_size,key_length,key_size,c1,c2,ratio" \
    > ../stat/${GENOME_NAME}.paralog_adjacent.all-repeat.csv
cat ../yml/repeat.family.txt \
    | parallel -j 1 --keep-order "
        cat ../stat/${GENOME_NAME}.paralog_adjacent.{}.csv \
            | perl -nl -e '/^chr_length/ and next; print qq({},\$_)'
    " >> ../stat/${GENOME_NAME}.paralog_adjacent.all-repeat.csv
cat ../yml/repeat.family.txt \
    | parallel -j 1 --keep-order "rm ../stat/${GENOME_NAME}.paralog_adjacent.{}.csv"
